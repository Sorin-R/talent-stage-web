...
const SWIPE_READY_TIMEOUT_MS = 2200;
const TAP_SUPPRESS_AFTER_SWIPE_MS = 360;
...
export default function Home({ onNav }: Props) {
  ...
  const activeSwipeTxnRef = useRef(0);
  ...
  const aheadPreloadRef = useRef<HTMLVideoElement | null>(null);
  const aheadPreloadedVideoId = useRef<string | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      const ahead = aheadPreloadRef.current;
      if (ahead) {
        ahead.pause();
        ahead.removeAttribute('src');
        ahead.load();
        aheadPreloadRef.current = null;
        aheadPreloadedVideoId.current = null;
      }
  ...
  const getPlayableIndexAtOffset = useCallback((offset: number): number | null => {
    if (feedVideos.length === 0 || offset <= 0) return null;
    let found = 0;
    for (let i = 1; i <= feedVideos.length; i++) {
      const idx = (feedIndex + i) % feedVideos.length;
      if (failedVideos.current.has(feedVideos[idx].id)) continue;
      found += 1;
      if (found === offset) return idx;
    }
    return null;
  }, [feedVideos, feedIndex]);
  ...
  // Keep one extra video (N+2) warmed in a detached element for fast swipes.
  useEffect(() => {
    const aheadIdx = getPlayableIndexAtOffset(PRELOAD_AHEAD_COUNT);
    if (aheadIdx === null || aheadIdx === feedIndex) return;
    const aheadVideo = feedVideos[aheadIdx];
    if (!aheadVideo || aheadPreloadedVideoId.current === aheadVideo.id) return;

    const preloader = aheadPreloadRef.current ?? document.createElement('video');
    aheadPreloadRef.current = preloader;
    aheadPreloadedVideoId.current = aheadVideo.id;
    preloader.muted = true;
    preloader.preload = 'metadata';
    preloader.src = aheadVideo.file_url;
    preloader.load();
  }, [feedIndex, feedVideos, getPlayableIndexAtOffset]);
  ...
  const onGestureEnd = useCallback((didSwipe: boolean) => {
    ...
    if (didSwipe) {
      suppressTapUntilRef.current = Date.now() + TAP_SUPPRESS_AFTER_SWIPE_MS;
  ...
  const finalizeSwipe = useCallback((txn?: number) => {
    ...
    const cancelAndRestore = (markFailed = false) => {
      ...
      setIsAnimating(false);
      setNextVideoReady(false);
      if (markFailed) failedVideos.current.add(nextVideo.id);
      const active = getActiveRef().current;
    ...
    // Hold until ready; if it stalls, cancel swipe and keep current video.
    if (preloadWaitTimer.current) clearTimeout(preloadWaitTimer.current);
    preloadWaitTimer.current = setTimeout(() => {
      ...
      cancelAndRestore(false);
    }, SWIPE_READY_TIMEOUT_MS);
  ...
  const handleSwipeTransitionEnd = useCallback((e: React.TransitionEvent<HTMLDivElement>) => {
    ...
    if (slideTimer.current) clearTimeout(slideTimer.current);
    finalizeSwipe(activeSwipeTxnRef.current || undefined);
